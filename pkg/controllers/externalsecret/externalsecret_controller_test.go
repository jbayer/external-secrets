...
var _ = Describe("ExternalSecret controller", func() {
	...
	deleteSecretPolicy := func(tc *testCase) {
		...
		tc.checkSecret = func(es *esv1beta1.ExternalSecret, secret *v1.Secret) {
			...
			fakeProvider.WithGetAllSecrets(map[string][]byte{}, v1beta1.DeleteSecretErr)
			Eventually(func() bool {
	...
	deleteSecretPolicyNone := func(tc *testCase) {
		expVal := []byte("1234")
		// set initial value
		fakeProvider.WithGetAllSecrets(map[string][]byte{
			"foo": expVal,
		...
		tc.externalSecret.Spec.Target.DeletionPolicy = esv1beta1.DeletionPolicyNone
		tc.externalSecret.Spec.RefreshInterval = &metav1.Duration{Duration: time.Second}
		tc.checkSecret = func(es *esv1beta1.ExternalSecret, secret *v1.Secret) {
			Expect(secret.Data["foo"]).To(Equal(expVal))
	...
	deleteSecretPolicyMerge := func(tc *testCase) {
		...
		tc.checkSecret = func(es *esv1beta1.ExternalSecret, secret *v1.Secret) {
			...
			fakeProvider.WithGetSecret(nil, v1beta1.DeleteSecretErr)
			Eventually(func() bool {
	...
	DescribeTable("When reconciling an ExternalSecret",
		...
		Entry("should not delete target secret with deletionPolicy=None", deleteSecretPolicyNone),
